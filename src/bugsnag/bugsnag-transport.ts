...
export type BugsnagTransportConfig = TransportConfig & {
...
export class BugsnagTransport extends Transport {
  ...
  private readonly logLevelToSeverity = new Map<
    WinstonLogLevel,
    BugsnagLogSeverity
  >([
    ['error', 'error'],
  ...
  log(info: LoggingInfo, next: () => void) {
    ...
    const notifyOptions: INotifyOpts = {
    ...
    this.client.notify(new Error(info.message), function (event) {
      event.addMetadata('metadata', notifyOptions);
    });
    return next();
  }
  ...
  private shouldLog(level: WinstonLogLevel): boolean {
    ...
    if (
      transportLevelPriority === undefined ||
      logLevelPriority === undefined
    ) {
      return true;
    }
